...
pub fn decode(bytes: &[u8]) -> Result<Operation> {
    ...
    let checksum = read_u32_le(&bytes[0..CHECKSUM_LEN]);
    let body = &bytes[CHECKSUM_LEN..];

    let record_type = body[0];
    let key_len = read_u32_le(&body[1..5]) as usize;
    let value_len = read_u32_le(&body[5..9]) as usize;
...
mod tests {
    ...
    fn truncated_header_fails() {
        ...
        .unwrap();
        assert!(matches!(decode(&encoded[..5]), Err(Error::CorruptedRecord)));
    }
